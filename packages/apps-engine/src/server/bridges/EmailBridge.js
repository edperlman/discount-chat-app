...
class EmailBridge extends BaseBridge_1.BaseBridge {
    doSendEmail(email, appId) {
    ...
    hasWritePermission(appId) {
        if (AppPermissionManager_1.AppPermissionManager.hasPermission(appId, AppPermissions_1.AppPermissions.email.send)) {
            return true;
        }
        AppPermissionManager_1.AppPermissionManager.notifyAboutError(new PermissionDeniedError_1.PermissionDeniedError({
            appId,
            missingPermissions: [AppPermissions_1.AppPermissions.email.send],
        }));
        return false;
    }
}
exports.EmailBridge = EmailBridge;
