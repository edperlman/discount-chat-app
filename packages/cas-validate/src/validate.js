...
function parseJasigAttributes(elemAttribute, cheerio) {
...
function parseRubyCasAttributes(elemSuccess, cheerio) {
...
function parseAttributes(elemSuccess, cheerio) {
    const elemAttribute = elemSuccess.find('cas\\:attributes').first();
    const isJasig = (elemAttribute === null || elemAttribute === void 0 ? void 0 : elemAttribute.children().length) > 0;
...
function validate(options, ticket, callback, renew = false) {
    if (!options.base_url) {
        throw new Error('Required CAS option `base_url` missing.');
    }
    const casUrl = url_1.default.parse(options.base_url);
    ...
    const query = Object.assign({ ticket,
        service }, (renew ? { renew: 1 } : {}));
    const queryPath = url_1.default.format({
    ...
    const req = https_1.default.get({
        ...
        res.on('data', function (chunk) {
            ...
            if (response.length > 1e6) {
                (_a = req.connection) === null || _a === void 0 ? void 0 : _a.destroy();
            }
        });
        res.on('end', function () {
            if (version < 2.0) {
            ...
            const cheerio = (0, cheerio_1.load)(response);
