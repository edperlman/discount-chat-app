...
const e2eeData = {
    userE2EE: {
        server: {
            ...
            public_key: '{"alg":"RSA-OAEP-256","e":"AQAB","ext":true,"key_ops":["encrypt"],"kty":"RSA","n":"qVLMv2Iwm_Hyhlnh4etNlHEiCXBzJWbqMwOZ6pz_JuZY2HiqbLmSfBtpvwBKZvcmP92BqLl-qZuLV_bJD_11UBS3gR6ykgU-RsTz1L-V8vA2QZEyULP4DqkMcRCV_7WE_sn_ScePgKszx1284gnngct_1Tv37zB6Ifz7gb1THRwAqOGcE2htea4yQEhyX8ZAl_-95DTWLbXqEAuofqDpXMcQo487VezBWIaDdfw2VX0qi6kM-pt03Gx8uMniyAjhK1G8Dro3wgAtz4PNIwOsdXEvWTSyoXLVMsIuZeO9OGdJKXnZFtVEMzXLyQTD1LjXlsM_TF09fbkN41Tz12ojmQ"}',
        },
        client: {
...
function generateContext(username) {
    var _a, _b;
    const date = new Date();
    date.setFullYear(date.getFullYear() + 1);
    const token = {
        token: crypto_1.default.createHash('sha256').update(username).digest('base64'),
        when: date,
    };
    const hashedToken = crypto_1.default.createHash('sha256').update(token.token).digest('base64');
    return {
        data: Object.assign({ _id: username, username, loginToken: token.token, loginExpire: token.when, hashedToken }, (e2eeData[username] && {
            e2e: (_a = e2eeData[username]) === null || _a === void 0 ? void 0 : _a.server,
                    ...
                    localStorage: [
                        ...
                        ...(e2eeData[username]
                            ? [
                                {
                                    name: 'private_key',
                                    value: (_b = e2eeData[username]) === null || _b === void 0 ? void 0 : _b.client.private_key,
                                },
                            ]
                            : []),
                    ],
...
function storeState(page, user) {
    return __awaiter(this, void 0, void 0, function* () {
        user.state.origins = (yield page.context().storageState()).origins;
    });
}
function restoreState(page_1, user_1) {
    return __awaiter(this, arguments, void 0, function* (page, user, options = {}) {
        var _a;
        let ls = user.state.origins[0].localStorage;
        if ((_a = options.except) === null || _a === void 0 ? void 0 : _a.length) {
            ls = ls.filter(({ name }) => { var _a; return !((_a = options.except) === null || _a === void 0 ? void 0 : _a.includes(name)); });
        }
        yield page.evaluate((items) => {
            items.forEach(({ name, value }) => {
                window.localStorage.setItem(name, value);
            });
        }, ls);
        yield page.waitForTimeout(2000); // Wait for the login to be completed
    });
}
