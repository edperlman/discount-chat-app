...
const parseMessageSearchQuery_1 = require("../../../../server/lib/parseMessageSearchQuery");
const data_1 = require("../../../mocks/data");
describe('parseMessageSearchQuery', () => {
    const params = {
        user: (0, data_1.createFakeUser)(),
    };
    ...
    [
        ...
        it(`should parse ${JSON.stringify(text)}`, () => {
            const { query, options } = (0, parseMessageSearchQuery_1.parseMessageSearchQuery)(text, params);
            (0, chai_1.expect)(query).to.deep.equal(expectedQuery);
            (0, chai_1.expect)(options).to.deep.equal(expectedOptions);
        });
    });
});
