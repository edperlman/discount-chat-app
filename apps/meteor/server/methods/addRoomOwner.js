...
meteor_1.Meteor.methods({
    addRoomOwner(rid, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(yield (0, hasPermission_1.hasPermissionAsync)(uid, 'set-owner', rid)) && !isFederated) {
            ...
            if (subscription.roles && Array.isArray(subscription.roles) === true && subscription.roles.includes('owner') === true) {
                throw new meteor_1.Meteor.Error('error-user-already-owner', 'User is already an owner', {
                    method: 'addRoomOwner',
                });
            }
            const addRoleResponse = yield models_1.Subscriptions.addRoleById(subscription._id, 'owner');
            ...
            yield core_services_1.Message.saveSystemMessage('subscription-role-added', rid, user.username, fromUser, { role: 'owner' });
            const team = yield core_services_1.Team.getOneByMainRoomId(rid);
            if (team) {
                yield core_services_1.Team.addRolesToMember(team._id, userId, ['owner']);
            }
            const event = {
                type: 'added',
                _id: 'owner',
